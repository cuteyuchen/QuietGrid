...
美股代币休市窗口自适应网格交易系统。v2 在原有时间窗口和强制离场基础上，加入 Market Regime、Inventory Manager、自适应非等距网格、分层风险预算、事件审计、保守回测和任务式 Vue 控制台。系统只在市场状态、成本和风险预算同时允许时启动网格。
...
- Regime 对波动、趋势、流动性、均值回归、成本和事件风险评分，并设置硬阻断；
- 网格中枢、区间、格距、格数和数量权重由市场状态与成本地板共同计算；
- Inventory 按 lot 追踪未配对库存，高风险时抑制同向加仓并优先减仓；
- Risk Manager 统一管理有效杠杆、会话/窗口损失、止损次数、强制离场和恢复条件；
- 提高风险的运行时配置不会热生效；
- 回测中心包含保守成交、未来数据校验、Walk-Forward、Monte Carlo、样本冻结和敏感性分析；
- Vue 控制台按“运行 / 研究 / 系统”组织完整功能，易用性不以删减功能为代价。

完整交付矩阵与已知边界见 [`docs/v2-implementation-status.md`](docs/v2-implementation-status.md)。

---

## 实现现状

v1.0 规划的 M1–M9 九个模块已经落地；v2 核心、控制台、回测验证和策略重放已按阶段提交。相较原始 plan，目录从单一 `core/` 拆分为分层包结构，Web 界面从 Streamlit 只读版升级为 Vue 3 + FastAPI 的交易控制中心。
...
SQLite（`db/database.py` 建表，`db/repository.py` 读写封装，开启 WAL）。v2 共 21 张表，包括 v1 运行表与 v2 事件、快照、参数、回测、命令和审计表：
...
| `event_store` | 事件存储与策略重放来源 |
| `feature_snapshots` / `regime_decisions` | 特征与市场状态决策 |
| `grid_plans` | 自适应网格计划版本 |
| `inventory_lots` / `inventory_snapshots` | 库存 lot 与风险快照 |
| `risk_snapshots` | 风控决定与预算快照 |
| `parameter_versions` | 参数版本元数据 |
| `backtest_runs` / `backtest_metrics` | 回测运行和指标 |
| `control_commands` / `audit_logs` | 幂等命令与操作审计 |
...
页面：总览、市场状态、会话与库存、风险中心、回测中心、策略重放、记录与订单对账、策略设置、运维与验收。

操作：暂停/恢复新开仓、请求关闭会话、停止全部会话、安全清扫、保存低风险参数草稿、触发只读环境校验/有界运行，以及通过 systemd（或自定义命令）控制交易 loop。危险动作统一二次确认、幂等提交并记录审计；高风险 v2 政策只读展示，不允许在运行中直接提高风险。
