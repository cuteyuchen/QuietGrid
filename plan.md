...
- [ ] M7 风控模块
- [ ] M8 SQLite 数据持久化
- [ ] M9 Web 监控界面（Streamlit 只读版）
- [ ] 测试网全链路测试
- [ ] 实盘小资金验证（2~3个窗口）

### 10.2 v1.1 TODO（优先级高）

- [ ] 钉钉/Telegram 通知：成交、止盈止损、击穿、程序异常等关键事件推送
- [ ] Web 界面操作功能：在网页上手动停止/启动某标的网格
- [ ] Web 界面参数修改：不重启进程修改参数（止盈、并发数等）
- [ ] 回测模块：用历史休市时段数据验证策略表现
- [ ] 费率监控：自动检测 Maker 零费率活动是否仍有效，费率变化时告警

### 10.3 v1.2 TODO（中期优化）

- [ ] 滚动重算区间：每隔一段时间（如2小时）用最新数据滚动更新区间，从静态网格进化为自适应网格
- [ ] Web 界面手动平仓：安全的手动平仓操作，带二次确认
- [ ] 多账户支持：同时管理多个 API Key 的账户
- [ ] 性能优化：高并发下的订单管理和数据库写入优化
