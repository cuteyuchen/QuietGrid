...
HOUR_MS = 60 * 60 * 1000
PROTOCOL_PATH = Path(
    "reports/cross-era-oos/round30-extreme-reversal-phase-a-protocol.md"
)
PROTOCOL_SHA256 = "be2efe47910069967b3d61b8c2d190f04f4b5415d8f0ef3ba5c5117e6666efb5"
DATA_AUDIT_PATH = Path("reports/cross-era-oos/round30-extreme-reversal-data-audit.md")
DATA_AUDIT_SHA256 = "89dab73a4873b099e9792f36c3625471257ebdcc459b8ca8a5ea8231fb5a827f"
ROUND29_READER_SOURCE_SHA256 = (
    "de202d483cc8430ebc5c02b7fab9e8dbd17bd047edc078f8decae26985765da6"
)
...
ROUND22_FUNDING_READER_SOURCE_SHA256 = (
    "874736a38de1e6468f1589ea5df1cf91399272ed88fc0b65f2e4cf85c4f643d6"
)
...
SCREEN_LOOKBACKS = (4, 8, 12, 24, 48)
SCREEN_HOLDS = (4, 8, 12, 24, 48)
SCREEN_THRESHOLDS = (0.005, 0.01, 0.015, 0.02, 0.03)
SELECTED_DIRECTION = "CONTRARIAN"
SELECTED_LOOKBACK_HOURS = 12
SELECTED_HOLD_HOURS = 48
SELECTED_THRESHOLD = 0.03
SELECTED_CANDIDATE_ID = "EXTREME_REVERSAL_12H_3PCT_HOLD48_1X_V1"
SPLITS = {
    "DEVELOPMENT": {
        "segment": "HISTORY",
...
def _candidate_id(direction: str, lookback: int, hold: int, threshold: float) -> str:
    if direction == "CONTRARIAN":
        prefix = "EXTREME_REVERSAL"
    elif direction == "MOMENTUM":
        prefix = "EXTREME_MOMENTUM"
    else:
        raise ValueError(f"未知方向模式: {direction}")
    threshold_pct = f"{threshold * 100:g}".replace(".", "P")
    return f"{prefix}_{lookback}H_{threshold_pct}PCT_HOLD{hold}_1X_V1"


def _split_bounds(split: Mapping[str, Any]) -> tuple[int, int]:
    return (
...
def _read_price_data(
    ...
    reader_path = Path(round29.__file__).resolve()
    if _sha256(reader_path) != ROUND29_READER_SOURCE_SHA256:
        raise ValueError("Round 29 价格读取器源码哈希不一致。")
    ...
    if dict(segment_counts) != EXPECTED_SEGMENT_ROWS:
        raise ValueError("Round 30 价格 segment 行数不一致。")
    if len(rows) != EXPECTED_ROWS_PER_ASSET:
        raise ValueError("Round 30 价格总行数不一致。")
    return manifest, rows, audit


def _read_funding_data(
    config: Mapping[str, Any],
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    reader_path = Path(round22.__file__).resolve()
    if _sha256(reader_path) != ROUND22_FUNDING_READER_SOURCE_SHA256:
        raise ValueError("Round 22 funding 读取器源码哈希不一致。")
    ...
    if manifest.get("file_sha256") != config["funding_csv_sha256"]:
        raise ValueError("Round 30 funding CSV 哈希不一致。")
...
def _assert_alignment(prices: Mapping[str, Sequence[Mapping[str, Any]]]) -> dict[str, Any]:
    ...
    if [int(item["open_time"]) for item in prices["ETH"]] != reference:
        raise ValueError("Round 30 BTC/ETH open_time 未完全对齐。")
    return {
        "asset_count": len(EXPECTED_SYMBOLS),
...
def _build_signals(
    ...
    *,
    ...
    if direction not in SCREEN_DIRECTIONS:
        raise ValueError("Round 30 direction 无效。")
    if lookback_hours <= 0 or threshold <= 0:
        raise ValueError("Round 30 lookback/threshold 必须为正。")
    segment_rows = [
        item for item in rows if str(item["segment"]) == str(split["segment"])
    ]
    by_time = {int(item["open_time"]): item for item in segment_rows}
    start_ms, end_ms = _split_bounds(split)
    path = [
        item
    ...
    if len(path) != expected_hours or len(path) != int(split["signal_rows"]):
        raise ValueError("Round 30 signal 行数不一致。")
    if any(
        int(current["open_time"]) - int(previous["open_time"]) != HOUR_MS
        for previous, current in zip(path, path[1:])
    ):
        raise ValueError("Round 30 signal 路径不连续。")

    signals: list[dict[str, Any]] = []
    for row in path:
        execution_time = int(row["open_time"])
        source_times = [
            execution_time - offset * HOUR_MS
            for offset in range(lookback_hours + 1, 0, -1)
        ]
        if any(timestamp not in by_time for timestamp in source_times):
            raise ValueError("Round 30 signal 缺少完整因果 warmup。")
        first_close = float(by_time[source_times[0]]["close"])
        latest_close = float(by_time[source_times[-1]]["close"])
        return_pct = latest_close / first_close - 1.0
        if direction == "CONTRARIAN":
            target = -1 if return_pct >= threshold else (1 if return_pct <= -threshold else 0)
        else:
            target = 1 if return_pct >= threshold else (-1 if return_pct <= -threshold else 0)
        signals.append(
            {
                ...
                "warmup_row_count": len(source_times),
                "first_source_time": source_times[0],
                "latest_source_time": source_times[-1],
    ...
    audit = {
        ...
        "lookback_hours": lookback_hours,
        "warmup_row_count": lookback_hours + 1,
        "latest_source_precedes_execution": all(
            int(item["latest_source_time"]) < int(item["execution_time"])
            for item in signals
        ),
...
def _performance(
    ...
    *,
    ...
    sharpe = (
        statistics.fmean(daily_returns) / stdev * math.sqrt(365)
        if stdev > 0
        else None
    )
    ...
    return {
        ...
        "positive_calendar_month_ratio": (
            len(positive_months) / len(month_pnl) if month_pnl else 0.0
        ),
        "best_profitable_month_concentration": (
            max(positive_months) / positive_profit if positive_profit > 0 else 1.0
        ),
...
def _simulate(
    ...
    *,
    ...
    expected_hours = (end_ms - start_ms) // HOUR_MS + 1
    if len(path) != expected_hours or any(
        int(current["open_time"]) - int(previous["open_time"]) != HOUR_MS
        for previous, current in zip(path, path[1:])
    ):
        raise ValueError("Round 30 cell 小时路径不完整。")
    ...
    scheduled_completed_trade_hours: list[int] = []
    final_truncated_trade_count = 0
    ignored_signal_count = 0
    held_funding_event_count = 0
    ...
    def mark(price: float) -> float:
        unrealized = (
            position * quantity * (price - entry_price) if position else 0.0
        )
        return initial_capital + cash + unrealized
    ...
    for row in path:
        ...
        if funding is not None and position:
            ...
            cash += payment
            funding_pnl += payment
            held_funding_event_count += 1

        closed_this_hour = False
        if position and scheduled_exit_time is not None and timestamp == scheduled_exit_time:
            realized = position * quantity * (open_price - entry_price)
            cash += realized
            price_pnl += realized
            exit_cost = quantity * open_price * execution_cost_rate
            cash -= exit_cost
            execution_costs += exit_cost
            execution_events.append({"action": "CLOSE", "cost": exit_cost})
            completed_trade_count += 1
            scheduled_completed_trade_hours.append(hold_hours)
            position = 0
            quantity = 0.0
            entry_price = 0.0
        ...
        if signal is not None and int(signal["target_position"]) not in {-1, 0, 1}:
            raise ValueError("Round 30 signal target 无效。")
        ...
        if signal is not None and int(signal["target_position"]) and not position and not closed_this_hour:
            ...
            scheduled_exit_time = timestamp + hold_hours * HOUR_MS
    ...
    checks = {
        ...
        "funding_mapping_coverage_100pct": (
            len(funding_by_hour) == selected_funding_event_count
        ),
        "all_signals_causal": all(
            int(item["warmup_row_count"]) == int(item["lookback_hours"]) + 1
            and int(item["latest_source_time"]) < int(item["execution_time"])
            for item in signals
        ),
        "all_execution_sides_costed_and_flat": (
            position == 0
            and len(execution_events) == 2 * completed_trade_count
            and all(
                math.isfinite(float(item["cost"])) and float(item["cost"]) > 0
                for item in execution_events
            )
        ),
        "completed_trade_hours_exact_when_scheduled": all(
            hours == hold_hours for hours in scheduled_completed_trade_hours
        ),
    ...
    return {
        "metrics": {
            **performance,
            ...
            "scheduled_completed_trade_count": len(scheduled_completed_trade_hours),
            "final_truncated_trade_count": final_truncated_trade_count,
            "ignored_signal_count": ignored_signal_count,
            "held_funding_event_count": held_funding_event_count,
            "selected_funding_event_count": selected_funding_event_count,
            "funding_mapping_coverage_ratio": (
                len(funding_by_hour) / selected_funding_event_count
                if selected_funding_event_count
                else 1.0
            ),
...
def _screen_development(
    ...
    for direction in SCREEN_DIRECTIONS:
        for lookback in SCREEN_LOOKBACKS:
            for hold in SCREEN_HOLDS:
                for threshold in SCREEN_THRESHOLDS:
                    candidate_id = _candidate_id(direction, lookback, hold, threshold)
                    symbols: dict[str, Any] = {}
                    for asset, config in ASSET_CONFIG.items():
                        signals, _audit = _build_signals(
                            prices[asset],
                            split,
                            direction=direction,
                        ...
                        symbols[asset] = _simulate(
                            ...
                            execution_cost_rate=SCENARIO_COST_RATES["COST50"],
                            hold_hours=hold,
                        )
                    metrics = {
                        asset: symbols[asset]["metrics"] for asset in ASSET_CONFIG
                    }
                    eligible = all(
                        float(metrics[asset]["total_pnl"]) > 0 for asset in ASSET_CONFIG
                    )
                    min_pnl = min(
                        float(metrics[asset]["total_pnl"]) for asset in ASSET_CONFIG
                    )
                    min_sharpe = min(
                        float(metrics[asset]["daily_annualized_sharpe"])
                        if metrics[asset]["daily_annualized_sharpe"] is not None
                        else float("-inf")
                        for asset in ASSET_CONFIG
                    )
                    min_month_ratio = min(
                        float(metrics[asset]["positive_calendar_month_ratio"])
                        for asset in ASSET_CONFIG
                    )
                    candidates.append(
                        {
                            ...
                            "minimum_cost50_pnl": min_pnl,
                            "minimum_cost50_sharpe": min_sharpe,
                            "minimum_cost50_positive_month_ratio": min_month_ratio,
                            "symbols": symbols,
                        }
                    )
    eligible = [item for item in candidates if item["eligible_positive_pnl_both_assets"]]
    if not eligible:
        raise RuntimeError("Round 30 Development screening 没有双资产正收益候选。")
    ordered = sorted(
        eligible,
        key=lambda item: (
            -float(item["minimum_cost50_pnl"]),
            -float(item["minimum_cost50_sharpe"]),
            -float(item["minimum_cost50_positive_month_ratio"]),
    ...
    if selected["candidate_id"] != SELECTED_CANDIDATE_ID:
        raise RuntimeError(
            "Round 30 Development 选择结果改变: "
            f"{selected['candidate_id']} != {SELECTED_CANDIDATE_ID}"
        )
    return {
        "candidate_count": len(candidates),
        "eligible_positive_pnl_both_assets_count": len(eligible),
        "selection_rule": [
            "minimum_cost50_pnl_desc",
            "minimum_cost50_sharpe_desc",
            "minimum_cost50_positive_month_ratio_desc",
            "candidate_id_asc",
        ],
        ...
        for asset in ASSET_CONFIG:
            ...
            for scenario, cost in SCENARIO_COST_RATES.items():
                item = _simulate(
                    ...
                    signals,
                    split=split,
                    initial_capital=float(ASSET_CONFIG[asset]["capital"]),
...
def _report(payload: Mapping[str, Any]) -> str:
    ...
    for name, cell in payload["cells"].items():
        ...
        failed = ", ".join(
            key for key, passed in cell["checks"].items() if not passed
        )
        ...
        lines.append(
            "| `{name}` | {pnl:.4f} | {pf} | {sharpe} | {dd:.2%} | {months:.2%} | {trades} | {price:.4f} | {funding:.4f} | {cost:.4f} | {passed} | {failed} |".format(
                ...
                months=metrics["positive_calendar_month_ratio"],
                trades=metrics["completed_trade_count"],
    ...
    lines.extend(
        [
            "",
            f"通过单元：{payload['summary']['passed_cell_count']}/{payload['summary']['cell_count']}。",
            "",
...
def main() -> None:
    ...
    conclusion = (
        "EXTREME_REVERSAL_WORTH_EXECUTION_PREREGISTRATION：12/12 个严格单元全部通过；只允许继续冻结真实执行细节和独立稳健性。"
        if candidate_ready
        else "NO_PREREGISTERED_EXTREME_REVERSAL_CANDIDATE：至少一个严格单元失败，排除本协议定义的 12h/3%/48h 价格极端反转 family。"
    )
    payload = {
        "schema_version": 1,
        "generated_at": datetime.now(UTC).isoformat(),
        "assessment_type": "ROUND30_EXTREME_REVERSAL_PHASE_A",
        ...
        "input_hashes": {
            ...
            "round22_funding_reader_source_sha256": ROUND22_FUNDING_READER_SOURCE_SHA256,
        },
        ...
        "cells": cells,
        "summary": summary,
        "formal_round30_execution_preregistration_ready": candidate_ready,
    ...
    result_path = report_dir / "round30-extreme-reversal-results.json"
    report_path = report_dir / "round30-extreme-reversal-report.md"
        ...
        json.dumps(
            {
                ...
                "passed_cell_count": summary["passed_cell_count"],
