...
UTC = timezone.utc
PROTOCOL_NAME = "cross-era-inventory-conditioned-wind-down-round6-20260723"
ROUND5_PROTOCOL_SHA256 = (
    "bcdb8dca0ece46a54d8ba007ed5a197eca8089d615c7f73c6e57a30e5e5ac450"
)
...
def _candidate_id(threshold: float) -> str:
    return f"IW{round(threshold * 100):02d}"


def _inventory_wind_down_seed_worker(
    threshold: float,
    seed: int,
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="评估库存条件式提前 wind-down 的跨周期稳健性。"
    )
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "round6-inventory-conditioned-wind-down-protocol.md"
        ),
    )
    return parser
...
def main() -> None:
    ...
    if seeds != DEFAULT_SEEDS:
        raise ValueError("Round 6 必须使用协议冻结的六个种子。")
    if args.workers <= 0:
        raise ValueError("workers 必须大于 0。")

    report_dir = Path(args.report_dir).resolve()
    report_dir.mkdir(parents=True, exist_ok=True)
    output = report_dir / "round6-inventory-conditioned-wind-down-results.json"
    if output.exists():
        raise FileExistsError(f"Round 6 结果已存在，拒绝覆盖: {output}")

    round5_protocol_path = report_dir / "round5-early-wind-down-protocol.md"
    round5_result_path = report_dir / "round5-early-wind-down-results.json"
    if _sha256(round5_protocol_path) != ROUND5_PROTOCOL_SHA256:
        raise ValueError("Round 5 冻结协议已变化。")
    if _sha256(round5_result_path) != ROUND5_RESULT_SHA256:
        raise ValueError("Round 5 冻结结果已变化。")
    round5 = json.loads(round5_result_path.read_text(encoding="utf-8"))
    if round5.get("eligible_candidate_ids"):
        raise ValueError("Round 5 已有合格候选，不应执行 Round 6。")
    if round5.get("final_oos_status") != "SEALED_NOT_EVALUATED":
    ...
    if round5.get("datasets") != datasets:
        raise ValueError("当前冻结数据与 Round 5 不一致。")
    market_states = {
    ...
    with concurrent.futures.ProcessPoolExecutor(
            ...
            for scenario, cost in (("BASE", BASE_COST), ("COST50", COST_50)):
                ...
                for seed in seeds:
                    future = executor.submit(
                        _inventory_wind_down_seed_worker,
                        threshold,
                        seed,
                        split_ids,
                        cost,
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if threshold != expected_threshold or seed != expected_seed:
                raise RuntimeError("Round 6 worker 返回了错误任务标识。")
    ...
    for threshold in _registered_thresholds():
        ...
        for cell_name, split_name, _window_ids, cost in cells:
            ...
            filtered = _filtered_evidence_for_symbols(
                ...
                contexts[split_name],
                candidate_id=candidate_id,
                round_name="round6_inventory_wind_down",
                split_name=split_name,
            )
    ...
    result = {
        ...
        "source_sha256": {
            "round5_protocol": ROUND5_PROTOCOL_SHA256,
        ...
        "conclusion": (
            "库存条件式提前 wind-down 选出唯一候选；只允许先预注册 Final OOS 协议。"
            if selected
            else "NO_ROBUST_CANDIDATE：库存条件式提前 wind-down 未通过四单元与机制门槛。"
        ),
    }
    _write_json(output, result)
    (report_dir / "round6-inventory-conditioned-wind-down-report.md").write_text(
